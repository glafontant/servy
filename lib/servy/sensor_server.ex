defmodule Servy.SensorServer do
  ...
  def handle_info(:refresh, state) do
    ...
    sensor_data = run_tasks_to_get_sensor_data()
    new_state = %{state | sensor_data: sensor_data, refresh_interval: state.refresh_interval}
    schedule_refresh(state.refresh_interval)
    {:noreply, new_state}
  end

  def handle_info(unexpected, state) do
    IO.puts "Something unexpected #{inspect unexpected}"
    {:noreply, state}
  end
  ...
  defp run_tasks_to_get_sensor_data do
    ...
    task = Task.async(fn -> Servy.Tracker.get_location("Gillette Stadium") end)
    ...
    where_is_gillette = Task.await(task)

    %{snapshots: snapshots, location: where_is_gillette}
  end
end